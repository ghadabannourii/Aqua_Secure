...
@section('title', 'Statistiques détaillées — AquaSecure')
...
<div class="container mx-auto px-4 sm:px-6 py-8 max-w-7xl">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        ...
        <div class="glass rounded-2xl p-4 hover-lift">
            <div class="flex items-center justify-between mb-3">
                <div class="w-9 h-9 rounded-xl {{ $bgCl }} flex items-center justify-center">
                    <i data-lucide="{{ $kpi['icon'] }}" class="w-4 h-4 {{ $iconCl }}"></i>
                </div>
                <span class="text-xs font-semibold {{ $trendCl }} flex items-center gap-0.5">
                    <i data-lucide="{{ $up ? 'trending-up' : 'trending-down' }}" class="w-3 h-3"></i>
                    {{ abs($kpi['change']) }}%
                </span>
            </div>
            <p class="text-lg font-display font-bold text-white leading-tight">{{ $kpi['value'] }}</p>
            <p class="text-[11px] text-cyan-100/50 mt-1 leading-tight">{{ $kpi['label'] }}</p>
            <p class="text-[10px] text-cyan-100/30 mt-0.5">vs {{ $kpi['prev'] }}</p>
        </div>
        @endforeach
    </div>

    {{-- ══ ROW 1 : Consommation mensuelle + Incidents ══════════════════════ --}}
    <div class="grid lg:grid-cols-2 gap-6 mb-6">

        {{-- Consommation (Area chart via SVG) --}}
        ...
        {{-- Incidents + résolutions (Bar chart) --}}
        <div class="glass rounded-2xl p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
    ...
    {{-- ══ ROW 2 : Qualité + Donut incidents ══════════════════════════════ --}}
    <div class="grid lg:grid-cols-3 gap-6 mb-6">

        {{-- Qualité eau (Line chart) --}}
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="glass px-3 py-1 rounded-lg">
                    <span class="text-teal-300 text-sm font-bold">
                        {{ $monthly['quality'][count($monthly['quality'])-1] }}%
                    </span>
        ...
        <div class="glass rounded-2xl p-6">
                ...
                <div class="relative">
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center">
                        <span class="text-2xl font-display font-bold text-white">
                            {{ array_sum(array_column($incTypes, 'count')) }}
                        </span>
            ...
            <ul class="space-y-2">
                @foreach($incTypes as $it)
                <li class="flex items-center gap-2">
                    <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background:{{ $it['color'] }}"></span>
                    <span class="text-xs text-cyan-100/70 flex-1 truncate">{{ $it['type'] }}</span>
                    <span class="text-xs font-bold text-white">{{ $it['pct'] }}%</span>
                </li>
                @endforeach
            </ul>
        </div>
    ...
    {{-- ══ ROW 3 : Performance équipes + Pression ══════════════════════════ --}}
    <div class="grid lg:grid-cols-2 gap-6 mb-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <div class="flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full" style="background:{{ $team['color'] }}"></span>
                            <span class="text-sm font-semibold text-white">{{ $team['team'] }}</span>
                        </div>
                        <div class="flex items-center gap-4 text-xs text-cyan-100/60">
                            <span>{{ $team['interventions'] }} interv.</span>
                            <span>⏱ {{ $team['avg_time'] }}</span>
                            <span class="font-bold" style="color:{{ $team['color'] }}">{{ $team['score'] }}%</span>
                        </div>
                    </div>
                    <div class="h-2.5 bg-slate-950/60 rounded-full overflow-hidden">
                        <div class="h-full rounded-full bar-animate"
                             style="width:{{ $team['score'] }}%; background: linear-gradient(90deg, {{ $team['color'] }}99, {{ $team['color'] }});
                                    animation-delay: {{ $i * 0.12 }}s">
                        </div>
                    </div>
                    <div class="flex justify-between text-[10px] text-cyan-100/30 mt-1">
                        <span>{{ $team['resolved'] }}/{{ $team['interventions'] }} résolus</span>
                        <span>Taux: {{ round($team['resolved']/$team['interventions']*100) }}%</span>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="glass rounded-2xl p-6">
            ...
            <div class="flex justify-between mt-2 px-1">
                @foreach($weekly['labels'] as $lbl)
                <span class="text-[10px] text-cyan-100/40">{{ $lbl }}</span>
                @endforeach
            </div>
            {{-- Legend --}}
            <div class="flex flex-wrap gap-3 mt-4">
                @foreach($weekly['zones'] as $wz)
                <span class="flex items-center gap-1.5 text-[10px] text-cyan-100/60">
                    <span class="w-2.5 h-2.5 rounded-sm" style="background:{{ $wz['color'] }}"></span>
                    {{ $wz['name'] }}
                </span>
                @endforeach
            </div>
                    ...
                    <tr class="hover:bg-white/[.02] transition-colors">
                        <td class="py-3 pr-4">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-white font-semibold text-xs whitespace-nowrap">{{ $zone['name'] }}</p>
                                    <p class="text-cyan-100/40 text-[10px]">{{ number_format($zone['population'], 0, ',', ' ') }} hab.</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-3 pr-4 text-center">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold
                                ...
                                {{ $zone['status'] === 'normal' ? 'Normal' : ($zone['status'] === 'alert' ? 'Alerte' : 'Critique') }}
                            </span>
                        </td>
                        <td class="py-3 pr-4 text-right">
                            <span class="text-xs font-bold {{ $qc }}">{{ $zone['quality'] }}%</span>
                        </td>
                        <td class="py-3 pr-4 text-right">
                            <span class="text-xs text-white">{{ $zone['pressure'] }} bar</span>
                        </td>
                        <td class="py-3 pr-4 text-right">
                            <span class="text-xs text-cyan-300">{{ $zone['flowRate'] }} m³/h</span>
                        </td>
                        <td class="py-3 pr-4 text-right">
                            <span class="text-xs text-cyan-100/70">{{ number_format($zone['consumption'], 0, ',', ' ') }} m³</span>
                        </td>
                        <td class="py-3 pr-4 text-center">
                            @if($zone['incidents'] > 0)
                ...
                <tfoot>
                    <tr class="border-t border-white/10">
                        ...
                        <td class="pt-3 text-right text-xs font-bold text-white">
                            {{ round(array_sum(array_column($zones,'pressure'))/count($zones),1) }} bar
                        </td>
                        <td class="pt-3 text-right text-xs font-bold text-cyan-300">
                            {{ number_format(array_sum(array_column($zones,'flowRate')),0,',',' ') }} m³/h
                        </td>
                        <td class="pt-3 text-right text-xs font-bold text-cyan-100/70">
                            {{ number_format(array_sum(array_column($zones,'consumption')),0,',',' ') }} m³
                        </td>
