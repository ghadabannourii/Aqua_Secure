...
<div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
    ...
    <div class="glass rounded-2xl p-4 ring-1 {{ $kpi['ring'] }} relative overflow-hidden hover-lift fade-up group"
         style="animation-delay:{{ $i * 0.06 }}s">
        <div class="w-10 h-10 rounded-xl {{ $kpi['ib'] }} flex items-center justify-center mb-3">
            <i data-lucide="{{ $kpi['icon'] }}" class="w-5 h-5 {{ $kpi['ic'] }}"></i>
        </div>
        <p class="text-2xl font-display font-bold {{ $kpi['vc'] }} leading-tight">{{ $kpi['val'] }}</p>
        ...
        <div class="absolute bottom-0 left-0 right-0 h-0.5 opacity-0 group-hover:opacity-100 transition-opacity"
             style="background:linear-gradient(90deg,transparent,{{ ['#05bfdb','#2dd4bf','#38bdf8','#818cf8','#ef4444','#34d399'][$i] }}80,transparent)"></div>
    </div>
    @endforeach
</div>

{{-- ══════════════════════════════════════════════════════════
     §2 — CARTE RÉSEAU (pleine largeur)
...
<div class="glass rounded-2xl overflow-hidden fade-up" style="animation-delay:.1s">
    <div class="px-6 py-4 border-b border-white/5 flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="text-cyan-100/50 text-xs mt-0.5">{{ count($mapZones) }} zones surveillées · {{ $totalRec }} réclamations</p>
        </div>
...
{{-- ══════════════════════════════════════════════════════════
     §3 — ÉTAT RÉCLAMATIONS (3 colonnes cliquables)
══════════════════════════════════════════════════════════ --}}
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 fade-up" style="animation-delay:.15s">
    ...
    <button onclick="filterRec('{{ $col['filter'] }}')"
            class="glass rounded-2xl p-6 ring-1 {{ $col['ring'] }} flex flex-col items-center text-center
        ...
        <div class="w-14 h-14 rounded-2xl {{ $cs['ib'] }} flex items-center justify-center mb-3">
            <i data-lucide="{{ $col['icon'] }}" class="w-7 h-7 {{ $cs['ic'] }}
                {{ $col['color']==='red' ? 'animate-pulse' : '' }}"></i>
        </div>
        <p class="text-4xl font-display font-bold {{ $cs['nc'] }} mb-1">{{ $col['count'] }}</p>
...
{{-- ══════════════════════════════════════════════════════════
     §4 — TABLEAU RÉCLAMATIONS  +  ACTIVITÉ
══════════════════════════════════════════════════════════ --}}
<div class="grid lg:grid-cols-5 gap-6 fade-up" style="animation-delay:.2s">
    ...
    <div class="glass rounded-2xl overflow-hidden lg:col-span-3">
            ...
            <table class="w-full text-sm">
                ...
                <tbody id="rec-tbody" class="divide-y divide-white/[.04]">
                    ...
                    <tr class="trow transition-colors rec-row cursor-pointer"
                        ...
                        onclick="openRecModal({{ $loop->index }})">
                        <td class="px-4 py-3">
                        ...
                        <td class="px-3 py-3 hidden md:table-cell">
                            <span class="text-xs text-cyan-100/60">{{ $rec['zone'] }}</span>
                        </td>
                        <td class="px-3 py-3 hidden sm:table-cell">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-cyan-100/60 truncate max-w-[80px]">{{ $rec['citizen'] }}</span>
                            </div>
                        </td>
                        <td class="px-3 py-3 text-center">
                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold
                                         {{ $ps['bg'] }} {{ $ps['text'] }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $ps['dot'] }}
                                    {{ $rec['priority']==='critical' ? 'animate-pulse' : '' }}"></span>
                                {{ $ps['label'] }}
                            </span>
                        </td>
                        <td class="px-3 py-3 text-center">
        ...
        <div class="px-5 py-3 border-t border-white/5 flex items-center justify-between">
            <span class="text-[11px] text-cyan-100/35" id="rec-count">{{ $totalRec }} réclamations</span>
    ...
    <div class="glass rounded-2xl overflow-hidden lg:col-span-2">
        ...
        <div class="relative">
            ...
            <div class="divide-y divide-white/[.04]">
                @foreach(array_slice($funcAct, 0, 7) as $act)
                @php
                ...
                <div class="flex items-start gap-2.5 px-4 py-2.5 hover:bg-white/[.02] transition-colors">
                    <div class="w-7 h-7 rounded-lg {{ $actBg }} flex items-center justify-center shrink-0 mt-0.5 relative z-10">
                        <i data-lucide="{{ $act['icon'] }}" class="w-3 h-3 {{ $actTx }}"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-1.5 flex-wrap">
                            <span class="text-white text-xs font-semibold truncate">{{ $act['actor'] }}</span>
                            <span class="px-1.5 py-0.5 rounded text-[9px] font-bold {{ $roleActBg }} {{ $roleActTx }}">
                                {{ $roleLabel }}
                            </span>
                        </div>
...
{{-- ══════════════════════════════════════════════════════════
     §5 — GESTION DES COMPTES (tableau avec actions)
══════════════════════════════════════════════════════════ --}}
<div class="glass rounded-2xl overflow-hidden fade-up" style="animation-delay:.25s">
    <div class="px-6 py-4 border-b border-white/5 flex flex-wrap items-center gap-3 justify-between">
        <div>
            ...
            <p class="text-cyan-100/50 text-xs mt-0.5">
                ...
                <span class="text-teal-400">{{ $activeAccounts }} actifs</span> ·
                <span class="text-red-400">{{ $blockedAccounts }} bloqués</span>
            </p>
        </div>
        <div class="flex items-center gap-2">
            {{-- Filtre rôle --}}
            <div class="flex gap-1" id="acc-filters">
                @foreach(['all'=>'Tous', 'citizen'=>'Citoyens', 'technician'=>'Techniciens', 'manager'=>'Gestionnaires'] as $fval => $flbl)
                <button onclick="filterAccounts('{{ $fval }}')" data-af="{{ $fval }}"
                        class="acc-tab px-2.5 py-1 rounded-lg text-[11px] font-semibold border transition-all
                               {{ $fval==='all' ? 'tab-active' : 'tab-inactive' }}">
                    {{ $flbl }}
                </button>
                @endforeach
            </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody id="acc-tbody" class="divide-y divide-white/[.04]">
                ...
                @php $rs = $roleStyle[$acc['role']]; @endphp
                <tr class="trow transition-colors acc-row" data-role="{{ $acc['role'] }}" data-id="{{ $acc['id'] }}">
                    {{-- Utilisateur --}}
                    <td class="px-5 py-3">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-xl bg-gradient-to-br {{ $rs['avatar'] }} border {{ $rs['border'] }}
                    ...
                    {{-- Statut --}}
                    <td class="px-3 py-3 text-center">
                        <span id="status-badge-{{ $acc['id'] }}"
                              class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold
                                     {{ $acc['status']==='active'
                                        ? 'bg-teal-500/15 text-teal-300'
                                        : 'bg-red-500/15 text-red-300' }}">
                            <span class="w-1.5 h-1.5 rounded-full
                                {{ $acc['status']==='active' ? 'bg-teal-400' : 'bg-red-400' }}"></span>
                    ...
                    <td class="px-3 py-3 hidden lg:table-cell">
                        <span class="text-[11px] text-cyan-100/40 whitespace-nowrap">{{ $acc['last_seen'] }}</span>
                    </td>
                    {{-- Actions --}}
                    <td class="px-3 py-3">
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            <button id="action-btn-{{ $acc['id'] }}"
                                ...
                                <i data-lucide="{{ $acc['status']==='active' ? 'ban' : 'unlock' }}"
                                   class="w-3.5 h-3.5"></i>
                            </button>
                            @endif
                        </div>
    ...
    <div class="px-5 py-3 border-t border-white/5 flex items-center justify-between">
        <span class="text-[11px] text-cyan-100/35" id="acc-count">{{ $totalAccounts }} comptes</span>
        <a href="{{ route('admin.users.index') }}"
           class="text-xs text-cyan-400 hover:text-cyan-300 font-semibold transition-colors">
            Gestion complète →
        </a>
...
{{-- ══════════════════════════════════════════════════════════
     §6 — RÉPARTITION UTILISATEURS + TECHNICIENS
══════════════════════════════════════════════════════════ --}}
<div class="grid lg:grid-cols-2 gap-6 fade-up" style="animation-delay:.3s">
    ...
    <div class="glass rounded-2xl p-6">
        <div class="flex items-center justify-between mb-5">
            <div>
                ...
                <p class="text-cyan-100/50 text-xs mt-0.5">{{ $totalAccounts }} comptes enregistrés</p>
            </div>
        </div>
        @php
            $distData = [
        ...
        <div class="flex items-center gap-6">
            ...
            <div class="flex-1 space-y-3">
                    ...
                    <div class="h-1.5 bg-slate-950/60 rounded-full overflow-hidden">
                        <div class="h-full rounded-full bar-grow"
                             style="width:{{ $dTotal > 0 ? max(2, round($dr['count']/$dTotal*100)) : 0 }}%;
                                    background:{{ $dr['color'] }};
                                    animation-delay:{{ $loop->index * 0.12 }}s"></div>
                    </div>
                </div>
                @endforeach
                {{-- Bloqués --}}
                <div class="pt-3 border-t border-white/5 flex items-center justify-between text-xs">
                    ...
                    <span class="font-bold text-red-300">{{ $blockedAccounts }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="glass rounded-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-white/5">
            ...
            <p class="text-cyan-100/50 text-xs mt-0.5">
                {{ count(array_filter($technicians,fn($t)=>$t['status']==='on_mission')) }} en mission ·
                {{ count(array_filter($technicians,fn($t)=>$t['status']==='available')) }} disponibles
            </p>
        </div>
        <div class="divide-y divide-white/[.04]">
            ...
            <div class="flex items-center gap-3 px-5 py-3 hover:bg-white/[.02] transition-colors">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <span class="w-6 h-6 rounded-full {{ $ts['interv_bg'] }} {{ $ts['text'] }}
